<script>
(function () {
  var FLOOR = 1400, CEILING = 2500, SPACING = 175, BEAT = 180;
  var body = document.body;
  var objects = Array.prototype.slice.call(document.querySelectorAll('.loader-obj'));
  ...
  var settled = false, cardWatching = false;
  ...
  function setPhase(phase) {
    if (reduceMotion && (phase === 'hero' || phase === 'card')) {
      ...
      window.setTimeout(function () {
        ...
        window.setTimeout(function () { body.classList.remove('is-phase-fading'); }, 40);
      }, 110);
    } else {
      body.setAttribute('data-loader-phase', phase);
    }
    if (phase === 'hero' && !cardWatching) { cardWatching = true; watchCard(); }
  }
  ...
  function watchCard() {
    ...
    new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting && !inCard) { inCard = true; setPhase('card'); }
        else if (!entry.isIntersecting && inCard) { inCard = false; setPhase('hero'); }
      });
    }, { threshold: 0.45 }).observe(cardEl);
  }

  function finish() {
    if (settled) return;
    settled = true;
    objects.forEach(function (o) { o.classList.add('is-revealed'); });
    setProgress(objects.length);
    setPhase('exit');
    window.setTimeout(function () { setPhase('hero'); }, BEAT);
  }
  ...
  objects.forEach(function (obj, i) {
    chain = chain.then(function () {
      ...
      return Promise.all([decodes[i], fontsReady]).then(function () {
        if (settled) return;
        var minTime = Math.max(start + FLOOR * ((i + 1) / objects.length), start + i * SPACING);
        var wait = Math.max(0, minTime - performance.now());
        return new Promise(function (resolve) {
          window.setTimeout(function () {
            if (!settled) { obj.classList.add('is-revealed'); setProgress(i + 1); }
            resolve();
          }, wait);
        });
      });
